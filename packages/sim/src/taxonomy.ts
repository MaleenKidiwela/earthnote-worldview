/**
 * Cousin taxonomy: the canonical Entity/CausalEdge/Scenario registry that
 * the rest of the app reads. Coordinates are approximate centroids,
 * sufficient for globe markers; survey-grade positions belong to
 * geo-specific layers, not here.
 *
 * Lifted verbatim from apps/web/src/sim-stub.ts (Phase 2 of the Cousin
 * integration). Every CausalEdge is `calibrated: false` until cross-checked
 * against the engine modules in ./engine.
 */
...
export type EntityWithGeo = Entity & { lonLat: readonly [number, number] };
...
export const ENTITIES: ReadonlyMap<string, EntityWithGeo> = new Map(
  SEEDS.map((s) => [s.id, s as EntityWithGeo]),
);
...
export const EDGES: readonly CausalEdge[] = [
