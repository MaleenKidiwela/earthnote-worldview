...
export function DataReadout({ viewer, filterMode }: DataReadoutProps) {
  const [camera, setCamera] = useState({ lat: 0, lon: 0, alt: 0 });
  ...
  useEffect(() => {
    ...
    return () => {
      if (!viewer.isDestroyed()) viewer.scene.postRender.removeEventListener(onTick);
    };
  }, [viewer]);
